...
## Data Cleaning
### Missing Values
We grouped the datasets by **s**, **m**, **h**, and **Side** to match the datasets with the label. We planned to use the mean price, open price (the best price, i.e. Level 1 price), volume (i.e. contracts) for each second as our features. We used `na.interpolation` and `na.replace` from R package `imputeTS` to fix the price of each level and set the volume to 0 if the price and volume are not available at a certain time.
