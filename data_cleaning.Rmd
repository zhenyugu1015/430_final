...
toSeconds = function(file){
  ...
  colnames(buy_output) = c('buy_time','buy_open','buy_close','buy_low','buy_high','buy_volume','buy_weighted_avg')
  
  sell_vol = sell_sum = sell_contr = 0
  sell_output = matrix(ncol=7)
  
  for (i in 1:nrow(sell_input)){
    sell_vol = sell_vol + sum(unlist(lapply(sell_input[i,4:13], contract))
                            * unlist(lapply(sell_input[i,4:13], order)))
    ...
    if (i==nrow(sell_input)){
      ...
      sell_output = rbind(sell_output, c(sell_time,sell_open,sell_close,sell_low,sell_high,sell_vol,sell_weighted_avg))
      sell_vol = sell_sum = sell_contr = 0
  ...
  colnames(sell_output) = c('sell_time','sell_open','sell_close','sell_low','sell_high','sell_volume','sell_weighted_avg')
  result = merge(buy_output, sell_output, by.x='buy_time', by.y='sell_time', all.x=TRUE, all.y=TRUE)
  columns = c('time','buy_open','buy_close','buy_low','buy_high','buy_volume','buy_weighted_avg','sell_open','sell_close','sell_low','sell_high','sell_volume','sell_weighted_avg')
  write.csv(result, file = "ticker_1.csv", col.names=columns)
}
```
